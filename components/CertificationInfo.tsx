...
const AuditReport: React.FC<AuditReportProps> = ({ data, user, onLoginRequired, onClose, onUpgrade }) => {
  ...
  const handleExportCertificate = async () => {
    ...
    const opt: any = {
      ...
      image: { type: 'jpeg', quality: 1.0 },
      html2canvas: { scale: 4, useCORS: true, backgroundColor: '#FAF7ED', width: 800, height: 1750, windowWidth: 800, windowHeight: 1750 },
      jsPDF: { unit: 'mm', format: [212, 462], orientation: 'portrait', compress: true },
    };
    try { await html2pdf().set(opt).from(element).save(); }
