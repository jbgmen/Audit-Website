    ...
    <div className={`sticky top-0 z-[60] w-full transition-all duration-200 ${
        ...
        <div className="flex items-center gap-2.5 flex-shrink-0">
          {user ? (
            <>
              <button
                onClick={() => go('pricing')}
                className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 rounded-[4px] border text-[9px] font-black uppercase tracking-[0.1em] transition-opacity hover:opacity-75 bg-transparent cursor-pointer"
            ...
            <>
              <button onClick={() => go('auth')} className="hidden sm:block px-4 py-2 border border-slate-200 rounded-[7px] text-[9px] font-black uppercase tracking-[0.14em] text-slate-500 hover:border-slate-900 hover:text-slate-900 hover:bg-slate-50 transition-all bg-transparent cursor-pointer">
                Sign In
              </button>
              <div className="hidden sm:block w-px h-4 bg-slate-200"/>
            </>
          ...
          <button onClick={() => go('audit')} className="flex items-center gap-2 px-4 py-2 bg-slate-900 text-white rounded-[7px] text-[9px] font-black uppercase tracking-[0.14em] hover:bg-slate-800 transition-all cursor-pointer border-0" style={{ boxShadow:'0 1px 3px rgba(15,23,42,0.2)' }}>
          ...
          {/* Hamburger */}
          <button onClick={() => setMenuOpen(!menuOpen)} className="lg:hidden flex flex-col gap-[4px] justify-center items-center w-9 h-9 border-0 bg-transparent cursor-pointer">
      ...
      {menuOpen && (
        <div className="lg:hidden absolute top-full left-0 right-0 bg-white border-b border-slate-200 shadow-[0_24px_48px_rgba(15,23,42,0.1)] flex flex-col px-5 py-4 max-h-[calc(100vh-62px)] overflow-y-auto z-50">
          ...
          {NB_MAIN.map(l => (
            <button key={l.id} onClick={() => go(l.id)} className={`flex items-center justify-between w-full px-3.5 py-3 rounded-lg text-[11px] font-bold uppercase tracking-[0.06em] mb-1 cursor-pointer border-0 transition-colors ${activeView === l.id ? 'bg-slate-900 text-white' : 'text-slate-600 hover:bg-slate-50 bg-transparent'}`}>
              {l.label}
          ...
          {NB_RESOURCES.map(r => (
            <button key={r.id} onClick={() => go(r.id as View)} className={`flex items-center justify-between w-full px-3.5 py-3 rounded-lg text-[11px] font-bold uppercase tracking-[0.06em] mb-1 cursor-pointer border-0 transition-colors ${activeView === r.id ? 'bg-slate-50 text-slate-900' : 'text-slate-500 hover:bg-slate-50 bg-transparent'}`}>
              <span className="flex items-center gap-2.5">
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ opacity:0.4, flexShrink:0 }}><path d={r.icon}/></svg>
                {r.label}
              </span>
          ...
          {user
            ? <button onClick={() => go('profile')} className="flex items-center justify-between w-full px-3.5 py-3 rounded-lg text-[11px] font-bold text-slate-600 hover:bg-slate-50 transition-colors mb-2 cursor-pointer border-0 bg-transparent">
                <span className="flex items-center gap-2.5">
                  <span className="w-6 h-6 rounded-[5px] bg-slate-900 flex items-center justify-center text-[9px] font-black" style={{ color:'#d4a017' }}>{(user.name?.[0] || 'U').toUpperCase()}</span>
